...
2. **Redelivery guard:** `status` already `'completed'` → logged and skipped. **A `'paid'` row is RE-RUN (2026-09-18)** — until then it was a skip too, which left a finalize that had failed at `paid` forever with nothing else looking; `finalizeTaxIntake` is resumable now.
3. Stamps `stripe_payment_intent_id`, `paid_at = existing || now` (a redelivery never re-dates the payment, #470), `status='paid'`.
4. Calls `finalizeTaxIntake` ([utils/tax-intake-finalize.ts](C:/vfo-edge-functions/supabase/functions/vfo-admin-api/utils/tax-intake-finalize.ts)), which re-reads the row fresh (#427), latches on `tax_intake_requests.created_client_at` (#327), and — **resumable since 2026-09-18** — stamps `client_id` / `tax_plan_id` the instant each row exists and REUSES whichever a failed run already created, then creates the program-4 enrollment, the `clients` row (on a Direct intake with the member as PF: `assigned_pf` + `pf_member_number`), the `client_enrollments` row, the `client_tax_plans` row (`tax_route`; `deposit_team_share_status='N/A — No Deposit'` from birth when waived) and the Deposit Paid step, issues the deposit invoice + receipt pair, and drafts the confirmation email to whoever paid. **A finalize that returns `ok:false` or throws still answers Stripe 200 and raises the action-required `FAILURE_tax_intake_finalize` bell** to Jake (no `notification_rules` row, by decision; title `Tax intake finalize FAILED — intake <id>`); the 02:30 tax sweep's 4th pass retries every `paid` row older than 10 minutes and clears it on success. Every step is best-effort behind the money write; the block is try/caught so a failure is logged and the webhook still answers 200.

**The `checkout.session.expired` twin** (same positive `payment_kind` guard): flips the row `'pending' → 'expired'` for that session id and touches nothing else — a paid or completed row is never expired. Route B may resubmit on the same token afterwards — and, since 2026-09-18, even BEFORE this event lands: `tax_intake_link_submit` accepts a `pending` row with no PaymentIntent and no `paid_at`, expires the stale Checkout itself and 409s only if Stripe reports it complete; route A starts a fresh form. `checkout.session.expired` is subscribed on the primary endpoints already (SpecRev uses it).

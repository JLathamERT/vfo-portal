...
### Sub-branch A1½ — Tax intake deposit ($500, card or ACH) *(added 2026-09-17, v860; card-or-ACH since 2026-09-23, v886; `router/webhooks.ts` "TAX INTAKE DEPOSIT" block → `utils/tax-intake-deposit-webhook.ts`)*

**Trigger:** the member (route A), the client (route B) or a confirmed VFO Tax Diagnostic's payer paid the $500 tax intake deposit on the Checkout minted by the `/tax-deposit-pay` choice page's `tax_intake_deposit_checkout` — by card ($515.24 incl. the 2.9% + $0.30 fee) or ACH ($500) — see [tax-intake.md](tax-intake.md).

**Position:** immediately after the GC block and **AHEAD of every customer-keyed branch** (MAP 1 → TAX → Advisor → PIP → Accountant), **primary-account events only** (`fromPrimary`). This session has **no Stripe customer** — the client does not exist yet — so nothing in the cascade below can claim it, and the block is **positively guarded on the object's own `metadata.payment_kind === 'tax_intake_deposit'`** (`isTaxIntakeDepositMetadata`, #473) so it can never claim a retainer. The same keys (`pipeline=TAX`, `payment_kind`, `intake_id`, `member_number`, `deposit_method`) sit on the session and the PaymentIntent, so the block catches both `checkout.session.*` and `payment_intent.*` events. Every helper refuses a row not stamped on the primary account or whose `sandbox` disagrees with `event.livemode` (#485); every UPDATE carries its own status guard and `.select()`s what it moved (#327). Same shape as the Background Check request block.

**What it does, per event (the PaymentIntent's OWN status decides, #475):**
1. **`checkout.session.completed`** (no `payment_status` gate — an ACH session completes `unpaid`): row by `stripe_checkout_session_id`, fallback by `metadata.intake_id` only when that row has no session stamp (a row pointing at a newer session → nothing booked + the `FAILURE_tax_intake_deposit_ach` "superseded Checkout" bell, unless its PI is dead). `completed` → skipped; **`paid` → the resumable finalize is re-run** (a failed finalize recovers on the redelivery); already booked on this PI → skipped. Otherwise it fetches the PI (expanding `payment_method` + `latest_charge`):
   - **`succeeded` (card):** stamps `stripe_payment_intent_id`, `paid_at = existing || now` (#470), `status='paid'`, `deposit_payment_method_type`, `deposit_acct_last4`, `deposit_card_fee`, then calls `finalizeTaxIntake`.
   - **`processing` / `requires_action` (ACH):** booked **in flight** — PI, method, last4, `deposit_processing_at` (+ `deposit_bank_verification_pending_at` on `requires_action`); the row stays `pending`; **no case yet**. `requires_action` raises the FYI bell `TAX_intake_deposit_verification_pending` (Jake + Tracy). A bank payment drafts the ACH confirmation at submit (`TAX_deposit_processing[|client]`, or the `|ach_verify` variant; once-only on `deposit_ach_email_sent_at`) — the Tax retainer precedent (#287).
   - **anything else:** nothing booked; the dead session stamp is released so the next mint is not refused forever.
2. **`payment_intent.processing`** — a row booked on this PI: clears `deposit_bank_verification_pending_at`; a backstop confirmation only if the submit-time draft never landed.
3. **`payment_intent.succeeded`** — the ACH **settled**: a `pending` row booked on this PI with `deposit_processing_at` → `status='paid'`, `paid_at`, then `finalizeTaxIntake`. A card PI's own `succeeded` is skipped (already `paid`).
4. **`payment_intent.payment_failed` / `payment_intent.canceled` / `checkout.session.async_payment_failed`** — **only a row BOOKED on that PI and in flight** (#482; an abandoned page's cancelled PI, or a card decline retried inside Checkout, is ignored): released to `status='expired'` with `deposit_failed_at`, PI / session / in-flight / latch columns nulled (the payer's link works again), and the action-required **`FAILURE_tax_intake_deposit_ach`** bell.
5. **`checkout.session.expired`** — `pending` → `expired` for that session id, only while no PI is booked.

`finalizeTaxIntake` ([utils/tax-intake-finalize.ts](C:/vfo-edge-functions/supabase/functions/vfo-admin-api/utils/tax-intake-finalize.ts)) re-reads the row fresh (#427), latches on `tax_intake_requests.created_client_at` (#327), and — **resumable since 2026-09-18** — stamps `client_id` / `tax_plan_id` the instant each row exists and REUSES whichever a failed run already created, then creates the program-4 enrollment, the `clients` row (on a Direct intake with the member as PF: `assigned_pf` + `pf_member_number`), the `client_enrollments` row, the `client_tax_plans` row (`tax_route`; `deposit_team_share_status='N/A — No Deposit'` from birth when waived) and the Deposit Paid step, issues the method-aware deposit invoice + receipt pair (card-fee row when a fee was paid; an ACH prints its settle date), and drafts the confirmation email to whoever paid. **A finalize that returns `ok:false` or throws still answers Stripe 200 and raises the action-required `FAILURE_tax_intake_finalize` bell** to Jake (no `notification_rules` row, by decision; title `Tax intake finalize FAILED — intake <id>`); the 02:30 tax sweep's 4th pass retries every `paid` row older than 10 minutes and clears it on success. The block is try/caught so a failure is logged and the webhook still answers 200.

Route B may resubmit on the same token after an expiry or a released bank failure — and, since 2026-09-18, even BEFORE the expiry lands: `tax_intake_link_submit` accepts a `pending` row with no PaymentIntent, no `paid_at` and no ACH in flight, and the choice page's checkout expires any stale Checkout itself (409 if Stripe reports it complete, 500 if its state is unknown). `checkout.session.expired` is subscribed on the primary endpoints already (SpecRev uses it).

**Tables read:** `tax_intake_requests`, `members`. **Tables written:** `tax_intake_requests`, `notifications`, then everything `finalizeTaxIntake` writes (`member_enrollments`, `clients`, `client_enrollments`, `client_tax_plans`, `client_tax_progress`, `document_numbers`).
**Chains:** none by HTTP — finalize and the ACH confirmation draft run in-process.

> The two generic failure resolvers (Branch C's `checkout.session.async_payment_failed` resolver and the `payment_intent.payment_failed`/`.canceled` resolver) **skip `payment_kind=tax_intake_deposit`** explicitly: the session has no customer of ours, so they could only raise the "unmapped Stripe customer" bell.

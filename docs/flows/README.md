...
| [tax-intake.md](tax-intake.md) | The member-run tax intake (2026-09-17/18, v860–v864): one 37-question form with three routes — the member fills and pays a **$500 card deposit** on a customer-less Stripe Checkout (`payment_kind=tax_intake_deposit`), or emails the CLIENT a tokened link to the public `/tax-intake` page, or (Holistic) records answers for an existing MAP 1 client after its first payment. Waived once the member has two qualifying clients. `finalizeTaxIntake` creates the program-4 enrollment, client, junction row, tax plan and Deposit Paid step, issues the house invoice + receipt pair and drafts the confirmation to whoever paid. Then the Green/Red Light step is the single trigger for both deposit money movements: Proceed forwards **$250** to the planning group, Refund returns the $500 to the payer. |

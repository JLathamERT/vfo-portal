...
## Revised invoice after an amendment *(2026-09-15, v849/v850)*
...
**When.** NOT at the amend step (Jake, same day, after a first cut did exactly that): it rides with the **next client email** as a **separate message** — the **Client decision 1** email (`postreview-decision.ts`, Continue or Undecided; Stop-Refund sends none) for a Tax 4 amendment, the **implementation decision** email (`implement-decision.ts`) for a Tax 5b one — through `utils/tax-amended-invoice-chain.ts maybeChainAmendedInvoice`, once per amendment (`amended_invoice_sent_at` must be older than the stage's `fee_amended_at_*` stamp; a decision re-send does not re-issue, a second amendment does), best-effort (reported in the response as `amended_invoice`, never fails the decision). The amend step itself shows *"A revised invoice … will be drafted … when the Client decision 1 email is sent"*, then *"Revised invoice drafted <date> in the original invoice email thread"*. Hand re-send / plans amended before 2026-09-15: `scripts/tax-revised-invoice.ps1 -PlanId N` (Lana's plan 107 was drafted this way, in her thread).
...
**The REVISED invoice (2026-09-15, v849/v850).** A real amendment at Tax 4 or Tax 5b also re-issues the retainer invoice **under the same number** — see [Revised invoice after an amendment](#revised-invoice-after-an-amendment) below. The two "fresh invoice at the next payment" arms above are unchanged and still fire as well.

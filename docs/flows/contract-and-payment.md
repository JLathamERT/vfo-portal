...
**Member-pays (`pipeline_map1.member_paying_on_behalf = true`):** the email goes To the member / Cc the client on the member variant (template 115 on payment 1), and both PDFs bill the MEMBER — Bill To / Received From is the member's name + email, with a **"Client: <client name>"** line printed under it (`onBehalfOfName` in `utils/html-templates.ts` `generateInvoiceHTML` / `generateReceiptHTML`; 2026-09-23). On the client-pays path `onBehalfOfName` is null and the line is not rendered.

**Reissue (`reissue: true`, 2026-09-23).** Corrects an ALREADY-SENT pair after a template change without allocating anything new. Superadmin session only (`body.token` = an `admin_sessions` token for the superadmin, `login_type` admin; 401 otherwise), and the payment must have a sent pair (`invoice_number` + `rec{N}_number` + `rec{N}_status='Sent'`; 400 otherwise). It:
1. re-renders the invoice + receipt under the **same numbers** and each document's **original date** (`invoice_email_sent_at` / `rec{N}_email_sent_at`, falling back to today);
2. uploads NEW Drive files and overwrites `invoice_drive_id` / `rec{N}_drive_id` — the old files stay in the folder;
3. resolves recipients from the template that sent the original (`|first`, `|subsequent`, or the member-pays first variant), incl. the client's additional contacts on the client-pays path;
4. finds the thread that CARRIED the receipt PDF with `utils/gmail-thread.ts` `findThreadByAttachment` (Gmail search `filename:"REC-….pdf" to:<first recipient>`, oldest match) — the subject alone also matches the member-pays agreement email — falling back to a subject search on the original subject;
5. drafts a short "updated copy, the only change is …, no action needed" reply in that thread with the PDF(s) attached (invoice only on payment 1) via `deliverRaw` under the template name `CONTRACT_invoicereceipt_email|reissue` — that name has **no `email_templates` row**, so it is always a Draft.

It stamps no send/status column, allocates no number and skips the tax-intake ask. Runner: `vfo-edge-functions/scripts/map1-reissue-invoice-receipt.ps1 -Email <superadmin> -Passcode <password> -ClientId <id> [-PaymentNumber <N>, default 1]` (logs in, then calls the action; `-Email` / `-Passcode` default to `VFO_SMOKE_EMAIL` / `VFO_SMOKE_PASSCODE`). First use 2026-09-23: client 126 Aaron Quick (member Charles West 59046), INV-59046-001-0048 / REC-59046-001-0001, reviewed and sent by Jake.

**Tables read:** `pipeline_map1`, `clients`, `members`, `pipeline_sandbox_config`, `email_templates`, `document_numbers`, `admin_sessions` (reissue only).
**Tables written:** `document_numbers` (insert), `pipeline_map1` (invoice_number, recN_number, drive IDs, email_sent flags; a reissue writes only the two drive IDs).

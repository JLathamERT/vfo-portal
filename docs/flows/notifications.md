...
**The exceptions to the rule above — FOUR bells go to the allocated Tax Planner + Team Member on Direct, not the member.** A bell asks for a STEP, so it is addressed to that step's OWNER on every route (#521), and decision 29 made the client's tax planning decision, the pricing form and the extra-meeting outcome the allocated planner's on a Direct plan. So on Direct `TAX_client_decision_needed` (`save-task.ts`, on "ROI Presentation" answered), `TAX_roi_skipped_decision_needed` (both fire sites — `skip-roi-meeting.ts` and `save-task.ts`), `TAX_client_decided_yes` and `TAX_client_extra_meeting_requested` (`final-decision.ts`, v873 — these two were found still following the member on the first live fixture) go through `notifyAllocatedPlanner` with the planner-portal link, exactly how `TAX_planner_assess_needed` is addressed. Classic plans keep the assigned PF. Titles are identical on both routes, so each single clear site (`decision.ts` for the first two; `pricing.ts` / `extra-meeting.ts` clear every unread TAX bell on the client for the last two) retires either. A Direct plan with no allocated planner raises nothing at all. Census method when a step changes owner: grep `actions/` + `utils/` for the step NAME and the bell TITLE prefix, never the rule key alone, and treat every `taxPfRecipientsFor(` site as a candidate.
...
Every TAX notification routes to a specific person via [`utils/tax-notify.ts`](C:/vfo-edge-functions/supabase/functions/vfo-admin-api/utils/tax-notify.ts) — none use `recipient:'admin'`. Tax 3 / Setup-phase → **assigned PF** (`taxPfRecipients`, **Tracy** fallback — `TAX_OWNERS` is `[TRACY_EMAIL]` alone since Tim Gacsy left); Tax 4/5 client-decision FYIs → **assigned PF + the allocated tax planner AND the allocated team member** (`taxDecisionRecipients(pf, taxPlanPeopleEmails(plan).emails)` — widened to both slot-holders 2026-09-16, `plannerEmails` accepting `string | string[]`; Tracy fallback; EACH planner-portal row link-overridden into the planner portal); the Tax 4 meeting-passed bell → **the allocated planner alone** since #170 (its own trigger is the CALENDAR meeting date, not a business-day ladder; the old PF + planner + Tracy tier is dormant); the Tax 4/5 stall "reach out" escalations (default **4 business days**) → **assigned PF**. **Since 2026-09-21 "assigned PF" is resolved off the CLIENT ROW (`taxPfRecipientsFor` / `taxDecisionRecipientsFor`), so on a DIRECT plan it means the member** — see § Direct tax plans above. **Two PF asks were added the same day, on every plan:** `TAX_roi_meeting_confirm_needed` (rule sort 271 — the 5-minute `reminder-sweep-5min` job asks the PF to confirm a booked ROI meeting happened once its tz-correct instant has passed and "ROI Presentation" is unanswered; once per plan via `client_tax_plans.roi_confirm_bell_sent_at`; cleared by answering the step, a refund, a no-deposit stop or a skip) and `TAX_client_decision_needed` (sort 272 — `save-task.ts` the moment "ROI Presentation" is answered with no decision; cleared by any decision). Plus one FYI, `TAX_plan_stopped_no_deposit` (sort 270 — a waived plan stopped from the Tax Plan Red Light step). **2026-09-25:** the confirm ask is titled *"Confirm the TPOM happened for…"* and the PF's `TAX_assess_completed_pf` ask *"Download the TPOM presentation for…"* (both ROI-titled before; the clears retire both vintages, #411); and the **Rapid Route** added three Tax rules routed by `notifyRapidOwner` (assigned PF on classic, both planner slots on Direct) — `TAX_rapid_email_needed` (273, action-required), `TAX_rapid_question_asked` (274, action-required, no email to the PF, no question text in the message) and `TAX_rapid_client_decided_yes` (275, FYI). Gotchas #291 (the four-layer departed-staffer checklist) + #292 (the per-recipient `links` map). Full per-notification inventory: [tax-planning.md § Notification inventory](tax-planning.md#notification-inventory-tax-pipeline--audit-2026-06-09).
...
**A fifth relative, and it is not a `notifyJakeFailure` sibling: `TAX_amended_invoice_not_sent` (2026-09-17)** *(v: 2026-09-17)*. It belongs here because it answers the same question — *who finds out when an automated document does not go out?* — but it routes to the **assigned PF**, not to Jake, and it is a **dismissible FYI** rather than an action-required alert. `utils/tax-amended-invoice-chain.ts` raises it through `insertTaxNotifications` / `taxPfRecipients` whenever `automation_TAX_amendedinvoice` refuses (`ok:false`), throws, or skips for any reason other than the three routine ones — *"fee not amended"*, *"revised invoice already drafted for this amendment"*, *"no email drafted"* — which are the chain working correctly and stay silent. Title *"Revised invoice not sent for &lt;Client Name&gt;"*, message = the reason verbatim, link to the client's tax tab, `dedupe:"unread"` so a redelivered chain or a re-sent decision email cannot stack copies. Nothing clears it automatically: re-issuing the document by hand (`scripts/tax-revised-invoice.ps1 -PlanId N`) is a judgement call, so there is no completing handler to own a clear site (#365 does not apply to an FYI). It is fail-soft in the strongest sense — awaited inside its own try/catch and returning nothing, so it can neither change nor delay the value the chain reports (#176). **Why it exists:** a best-effort chain that refuses is invisible — `console.error` was the only trace, and Petrus Phoa's plan 71 sat refused for **eight days** before anyone knew (**#507**). **NEVER FIRED LIVE** as at 2026-09-17: testing ended before a refusal was provoked.

**Sixth sibling: `FAILURE_tax_deposit_team_share` (2026-09-17)** *(v: 2026-09-17)*. Raised through `notifyJakeFailure` by `utils/tax-deposit-team-share.ts` whenever the **$250 Tax Planning Team share of the $500 tax intake deposit** cannot be forwarded to the planning group's Connect account — the planner has no group, the group has no `stripe_account_id`, its transfers capability is not active (the `connectTransfersActive` probe, #489, runs before the key is spent), Stripe is unconfigured for the mode, or Stripe refused. Seeded by migration `20260917120000_tax_intake.sql` with the shape copied from `FAILURE_tax_planner_share`: area **Payment Failure Alerts**, sort **52**, `kind='bell'`, **`action_required=true`**, `default_recipients ["jlatham@elitert.com"]`, enabled, no delay. The row exists so the alert can be re-routed or disabled from the Notification Editor like every other bell; Jake is also the code default. One title per plan (`Tax deposit team share FAILED — <Client> (<ref>)`), `dedupe:"unread"`, the message carries the specific reason and *"The daily tax sweep will retry."*; `clearJakeFailure` retires it on the successful transfer, whether that comes from a re-saved review Proceed, `tax_allocate_planner` or the sweep's third pass. The rule's stored trigger note was re-worded by SQL at the 2026-09-21 wrap-up to name the review step's Proceed (it had still said Green/Red Light Proceed after the trigger moved — decision 27). Full leg: [tax-intake.md](tax-intake.md#the-250-tax-planning-team-share--the-review-steps-proceed). **Never fired live** — the one live Proceed on 2026-09-18 transferred first time.

**And one with NO rule row: `FAILURE_tax_deposit_docs` (2026-09-17)** *(v: 2026-09-17)*. `utils/tax-deposit-docs.ts` calls `notifyJakeFailure` with this key when a freshly allocated deposit invoice/receipt number is **not found in `document_numbers` after allocation** (#282 — `allocateDocNumber` returns its candidate even when the reservation insert failed); the documents are skipped rather than risk a duplicate accounting number and the confirmation email goes out without attachments. There is **no `notification_rules` row for this key**: `notifyByRule` finds no rule and falls back to the call's `defaultRecipients` — Jake — as an action-required bell that the Notification Editor can neither re-route nor disable. Title `Tax deposit document number not reserved — <Client>`, link to the client's tax tab, nothing clears it (the pair is issued by hand). Never fired.

**And a second with NO rule row: `FAILURE_tax_intake_finalize` (2026-09-18)** *(v: 2026-09-18)*. `router/webhooks.ts` calls `notifyJakeFailure` with this key when the resumable `finalizeTaxIntake` returns `ok:false` or throws on a `paid` intake row — the $500 cleared in Stripe and nothing downstream exists, and a `console.error` is not a notification (#507). Action-required, title `Tax intake finalize FAILED — intake <id>`, link `/admin?tab=automation`, message names the member and the error and says the nightly tax sweep will retry — which it does (pass 4, `paid` rows older than 10 minutes), clearing this title on success; the webhook clears it too on a later successful redelivery. Jake by code default, a decision. Never fired (the forced-failure proof was not run).

### Cross-pipeline: ACH manual bank entry ("verification pending") bells

One family, three pipelines, one shape: an ACH payer who **typed their account and routing numbers** instead of signing in to their bank leaves Stripe holding the money-movement object for micro-deposit verification, and **no money moves** until they enter the deposit code. Each pipeline records that sub-state in a nullable **side column** rather than a status value (**#475**) and raises a **dismissible, `dedupe:"unread"`** FYI so an admin can see the wait rather than reading a healthy-looking row.

| Rule key | Pipeline | Raised by | Default recipients |
|---|---|---|---|
| `MAP1_ach_bank_verification_pending` | MAP 1 payment 1 | `checkout.session.completed`, PaymentIntent `requires_action` | Jake + Tim |
| `TAX_ach_bank_verification_pending` | Tax retainer / final retainer | `checkout.session.completed`, PaymentIntent `requires_action` | Jake + Tim |
| `SPECIALIST_lic_bank_verification_pending` (2026-09-14) | Specialist **licence continuation** | `checkout.session.completed`, subscription's **`pending_setup_intent`** `requires_action` | Tracy |
...
- `load_notifications` LEFT `ADMIN_ONLY_ACTIONS` on 2026-09-21 (it had been there since the gate existed): admins, tax planners and members may read — a `tax_planner` or `member` session only its own rows (`OWN_EMAIL_ONLY_ROLES`, `recipient = session.email`; for a member that is `member_logins.email`, #519).
- `mark_notification_read` left with it — a planner or member may mark only a row addressed to them (`OWN_ROWS_ONLY_ROLES`; anything else 404s).
- `load_notifications_page` / `mark_notifications_read` (the page + bulk clear) stay `ADMIN_ONLY_ACTIONS`.
...
## Not here: per-client and per-member email Cc

**In-app bell recipients and email recipients are different mechanisms and this doc covers only the first.** There are now TWO Additional-Contact mechanisms and **neither touches a bell**:

- **Client contacts** (since 2026-08-20). `client_contacts.cc_on_emails` adds a **per-client leg to EMAIL recipient resolution** — `loadAdditionalContacts` → `resolveTemplateRecipients`'s `extraCc` parameter — and can be folded into the `[Client First]` body salutation. Login-setup / password-reset emails and the planner-facing assess reminders are deliberately excluded; the `send_mode=true` vault documentation request is wired and **real-sends**.
- **Member contacts** (since 2026-09-16). `member_contacts.cc_on_emails` is resolved **centrally inside `resolveTemplateRecipients`**: whenever the member's own email lands in **To or Cc** (never Bcc) of any templated send, their flagged contacts are appended to Cc. Admin-managed, one switch, no greeting toggle. Opted out of on the five credential handlers via `options.skipMemberContacts`.

**No bell recipient is affected by either**: an additional contact — client or member — never receives a notification, never appears in `notification_rules.default_recipients`, and never clears an action-required bell. **Member contacts are email-only, exactly like client contacts.** Full mechanism → [additional-contacts.md](additional-contacts.md).

...
## The $250 Tax Planning Team share — Green/Red Light Proceed
...
**Trigger — the single one:** `tax_save_task` ([actions/tax/save-task.ts](../../../vfo-edge-functions/supabase/functions/vfo-admin-api/actions/tax/save-task.ts)) when the saved task's `status_options` is the Green/Red sentinel (`tax_refund`) and the status is `Proceed`. Isolated in its own try/catch so a Stripe outage never fails the step save. **Applies to every deposit plan**, the nine hand-pasted legacy ones included — the leg keys on `deposit_payment_intent_id`, not on an intake row.

| Status | When | Terminal |
|---|---|---|
| `Yes` | transfer created | yes |
| `N/A — No Deposit` | no `deposit_payment_intent_id` (waived intake, or a plan that never had one). **Since 2026-09-21 a waived intake is stamped with it AT PLAN CREATION** (`utils/tax-intake-finalize.ts` sets the column on the insert), because such a plan has no Green/Red Light step to click Proceed on at all — see [tax-planning.md Step 0b](tax-planning.md#step-0b--tax-plan-greenred-light-decision-inside-tax-1---diagnostic). Terminal, so no trigger or sweep can ever revisit it | yes |
...
**Release on allocation:** [actions/tax/allocate-planner.ts](../../../vfo-edge-functions/supabase/functions/vfo-admin-api/actions/tax/allocate-planner.ts) — planner slot only (never the team-member slot), a real target, a PI on the plan, status not terminal, and `getGreenRedStatus(plan) === 'Proceed'`; response carries `deposit_team_share`. **Nightly retry:** the tax revshare sweep ([actions/tax/revshare-sweep.ts](../../../vfo-edge-functions/supabase/functions/vfo-admin-api/actions/tax/revshare-sweep.ts)) gained a third pass over rows in `Failed` / `Awaiting Planner Allocation` with a PI, each gated on the Green/Red step actually reading `Proceed`; reported as `deposit_team_retried`.

In the UI the allocation step (Tax 1 order 2) sits above the Green/Red Light step (order 8), so `Awaiting Planner Allocation` is unreachable by normal clicking — it is the sweep's backstop and **has never fired live**.
...
- **Green/Red Light** (`tax_refund`): **listed on every surface since 2026-09-18** — until then the member view hid it unless a refund had landed. **A waived (no-deposit) plan does not carry the step at all since 2026-09-21**, so none of the below applies there. Member view is read-only: the outcome chip (`Proceed`, a red **Refunded $500.00**, or *Not started*); the buttons are gated on `!readOnly`. The **$250 team share chip** (`$250 team share paid` / `$250 team share failed` / `Awaiting Planner Allocation` / `N/A — No Deposit`) renders **admin-only** after Proceed — VFO's own money movement, hidden from `readOnly` and `plannerMode` like the pricing split card.

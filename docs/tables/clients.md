...
| `assigned_pf` | text | Planning Facilitator assigned to this client — a NAME, resolved to an email by the maps in `utils/pf-emails.ts` (correspondence) and `utils/tax-notify.ts` (portal-login bells). **On a DIRECT tax case (2026-09-18) it holds the MEMBER's display name** (written by `utils/tax-intake-finalize.ts`), which is in neither map — `pf_member_number` below is what makes that resolve. The Proactive Facilitator card renders it read-only with "(Member — Direct)" and offers no dropdown, because a select whose value has no option silently reads "-- Select --". |
| `pf_member_number` | text | **NEW 2026-09-18** (migration `20260918210000_clients_pf_member_number.sql`; nullable text, no FK, no index — the column comment on the table is the contract). **The member who is this client's Planning Facilitator on the DIRECT route** — set by `utils/tax-intake-finalize.ts` when the intake carried `tax_route='direct'`, NULL on every classic client. It is the discriminator behind #514: `utils/pf-emails.ts resolvePfEmail(sb, client)` returns that member's `members.email` when set (else the admin name map), and `utils/tax-notify.ts resolveTaxPf` returns their `member_logins.email` for bells (#519) — so **every `clients` select that feeds a PF email token or a PF bell must include it** (#448; an omission resolves as classic, silently). Also the Direct fence for a member's client-scoped actions: `denyIfNotDirectClient` (own client AND `pf_member_number` = the session member) guards `msm_add/update/delete_client_contact` and `tax_direct_note_add`; `ClientDetail.jsx` compares it against the session to enter `directMode`. The plan-side twin of the same fact is `client_tax_plans.tax_route` ([tax.md](tax.md)). |
...
| `tax_intake_requested_at` | timestamptz | **NEW 2026-09-17** (migration `20260917120000_tax_intake.sql`). **The Holistic once-only stamp**: set when the *"Please complete the Tax Planning Form"* email (`TAX_intake_request\|holistic`, template 276, To the MEMBER) is drafted after a Holistic client's FIRST MAP 1 payment settles. ONLY writer: `utils/tax-intake-request-email.ts`, called from `actions/pipeline/contract-invoice-receipt.ts` — a conditional `.is(null)` update, so it is the redelivery latch for that chain (#327/#470): a replayed webhook finds it set and drafts nothing. The email's button opens the member portal's Tax Planning tab with `?intake_client=<id>`, and the answers land in [`tax_intake_requests`](tax.md#tax_intake_requests-added-2026-09-17) against this existing client with no deposit. **Nothing reads it for behaviour beyond that guard**, and the trigger itself has never fired live (the Holistic entry was only ever exercised by URL on client 62). |
...
**Member callers since 2026-09-18:** all three contact writes require `denyIfNotDirectClient` for a `member` session — own client AND `clients.pf_member_number` = the session member (a Direct case the member runs); a member edits name/email only, the two switches stay admin-only. No member surface reached these before Direct. **Touched by:** `msm_add_client_contact`, `msm_update_client_contact` (2026-08-20), `msm_delete_client_contact`, `load_member_contacts`, `msm_load_client_home`, `ciq_add_client_and_create`, `msm_add_client`, and **every emailing handler that calls `loadAdditionalContacts`** (53 sites at ship — derive it, don't trust the count).

...
| `trading_name` | text | nullable. Added 2026-06-18. **"Company Name"** (UI label; the column name stays `trading_name`). Shown/editable on **accountant AND advisor** profiles (2026-07-14 — was accountant-only) — the Add-Accountant form + Edit Profile + profile details + the member's own portal. Auto-filled on onboarding create from the New-Model-Sale modal's `sale_company_name` (`advisor`/`accountant/create-member.ts`). Inserted by `add_member_full`, persisted by `member_profile_save` (passthrough upsert) **and, since 2026-09-24, by the member's own `member_self_profile_save`** (CR/LF/`<>` stripped, #532), returned by `load_data`. **Feeds the tax agreement** when `contract_name_mode='company'` (see below) — so `member_self_profile_save` refuses to CLEAR it while that mode is set. |
| `email` | text | **The ROUTING address — every outbound email to this member goes here**, and it is the only member address anything sends to. UI label since 2026-08-20: **"Work email (emails sent here)"** on Edit Profile / profile details, **"Work Email \*"** on the add + onboarding forms (`MembersPanel.jsx`, `AdvisorOnboarding.jsx`, `AccountantOnboarding.jsx`). Column name and every writer/reader unchanged — the relabel exists purely to make the split below unambiguous at the point of entry. **Since 2026-09-24 the member can change it themselves** on the portal's Edit Profile page (`member_self_profile_save`) — which moves where portal emails go but NOT the login email, which is `member_logins.email` and stays as it was (by design). |
...
| `notes` | text | |
| `headshot_image` | text | nullable. Added 2026-07-10. **Member profile headshot** — stores just the filename; the image lives in the public `headshots` bucket (shared with `experts.headshot_image`), rendered as `<HEADSHOT_SUPABASE>/<encoded filename>`. Admin: uploaded via `upload_headshot` + cropped by `ImageCropModal`, persisted by `member_profile_save` (passthrough upsert). **Member (2026-09-24):** their own Edit Profile page uploads a crop-modal JPEG (≤ 1.5 MB) through `member_self_profile_save`, server-named `<ts>_member_<num>.jpg`. Surfaced on the admin `TrackHero` avatar + member-portal hero via `load_data` (must be whitelisted there — gotcha #207). |
...
| `website_url` | text | nullable. Added 2026-07-10. Member website; rendered as a clickable link (bare domains get `https://` prepended). Edited by the admin via `member_profile_save` and, since 2026-09-24, by the member via `member_self_profile_save`. Auto-filled on advisor/accountant onboarding create from the New-Model-Sale modal's `sale_website` (2026-07-14). |
| `logo_image` | text | nullable. **Added 2026-09-24** (migration `20260924160000_branding_columns.sql`). The member's ROI-deck logo — bare filename of a server-named `logo_member_<num>_<ts>.png` in the PUBLIC `headshots` bucket (1200×400 PNG, ≤ 1 MB, white badge baked in by `src/components/shared/logoPng.js`). Written ONLY by `member_branding_save` (upload / `remove_logo`; the replaced `logo_member_` file is deleted best-effort). Read by `tax_generate_presentation` (bottom-middle on every slide) — only when `logo_enabled` is true. |
| `logo_enabled` | boolean | not null, default `false`. **Added 2026-09-24** (same migration). The member's *"Show my logo on ROI presentations"* switch; the deck reads the logo only when this is `=== true`. Written ONLY by `member_branding_save`. |
| `contract_name_mode` | text | nullable, CHECK in `('company','personal','none')`. **Added 2026-09-24** (migration `20260924190000_members_contract_name_mode.sql`). How the tax engagement agreement's collaborating-team sentence names the member (`actions/tax/send-agreement.ts`): `'company'` = `trading_name` (blank → left out), `'personal'` = first + last name, **`'none'` or NULL (never chosen) = left out**. UI: the three-way *Company name / Real name / Neither* buttons on the Branding card. Written ONLY by `member_branding_save`, which refuses `'company'` (400) while `trading_name` is blank. *(A boolean `contract_name_enabled` existed on 2026-09-24 only and was dropped by `20260924200000`.)* |
...
**Touched by:** `load_data`, `add_member`, `add_member_full`, `save_member`, `delete_member`, `member_profile_load`, `member_profile_save` (incl. `introduced_by_member_number` + `connection_type` — a full passthrough spread; **and since 2026-08-24 the hold-release trigger above, whose response gains an additive `payout_release` key when it fires**), `upload_headshot` (profile headshot → `headshots` bucket), **`member_self_profile_save`** and **`member_branding_save`** (2026-09-24 — see *Member branding* below), `tax_generate_presentation` / `automation_TAX_sendagreement` (read the branding columns), `automation_CONTRACT_revshare`, `automation_TAX_revshare`, `automation_PIP_revshare`, `specialist_revenue_payout`, `automation_MEMBERSHIP_sweep`; **and since 2026-08-27 `member_stripe_link_lead`** (the only writer of `stripe_account_linked_from`, and the only writer of `stripe_account_id` that does not come from Stripe account creation). Frontend: [MembersPanel.jsx](src/components/admin/MembersPanel.jsx), [MemberPortal.jsx](src/pages/MemberPortal.jsx), [corporateMember.js](src/components/shared/corporateMember.js) (shared corporate-lead helper), [MemberPaymentsTab.jsx](src/components/payments/MemberPaymentsTab.jsx) (the borrowed-account note in place of the payout table).

### Member branding: two narrow writers, and the branding never enters `load_data` (2026-09-24)

- **`member_branding_save`** (AUTH, `MEMBER_SCOPED_ACTIONS`, `actions/members/branding-save.ts`) writes ONLY `logo_image` (`logo_base64` upload or `remove_logo`), `logo_enabled` and `contract_name_mode`. A member always writes their own row (`auth.callerMemberNumber` — a body `member_number` is never read for them, #142), an admin passes `member_number`, every other role 403s. Used by the Branding card (`src/components/shared/MemberBrandingCard.jsx`) on the member portal's Edit Profile page and the admin Edit Profile tab.
- **`member_self_profile_save`** (AUTH, `MEMBER_SCOPED_ACTIONS`, member-only in-handler, `actions/members/self-profile-save.ts`) is the member's own Edit Profile: `email`, `trading_name`, `website_url`, headshot. Nothing else — status, type, holds, revenue decision and notes stay admin-only (`member_profile_save`).
- **`member_profile_save` (admin) DELETES `logo_image` / `logo_enabled` / `contract_name_mode` (and the dropped `contract_name_enabled`) from its payload before the upsert** — it echoes a whole profile, and an admin form left open must never overwrite branding the member set since (#490 family).
- **The `load_data` members whitelist is UNCHANGED** — the three branding columns are private to the member and admins (#445). Every member surface (Profile, Edit Profile, the admin Details tab via the shared `MemberProfileDetails.jsx`) reads them through **`member_profile_load`** (`select *`).

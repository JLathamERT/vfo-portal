...
|---|---|---|---|
| `headshots` | public | `upload_headshot`, `member_self_profile_save`, `member_branding_save`, `tax_planning_group_logo_save`; read by `tax_generate_presentation` | Member/specialist/tax-planner headshots (500×500 JPEG via `ImageCropModal`, 1-year cache), served via direct public URL. Backfilled 2026-07-24 (gotcha #278). **Also holds the ROI-deck logos (2026-09-24):** `logo_member_<num>_<ts>.png` (`members.logo_image`) and `logo_group_<id>_<ts>.png` (`tax_planning_groups.logo_image`) — see the `headshots` section below. |
...
| `tax-agreements` | public | `actions/tax/decision.ts` (Undecided branch) | Holds the four static **Tax Planning Engagement Agreement** sample PDFs — `tax-planning.pdf`, `tax-planning-member.pdf`, `tax-planning-3pay.pdf`, `tax-planning-member-3pay.pdf` — re-rendered from `agreement_templates` rows 25–28 and re-uploaded 2026-09-24. Fetched no-auth at email-draft time and attached as a multipart MIME part to the client's Undecided email. |
...
| `presentation-templates` | private | `ROI-template-master*.pptx` — read by `tax_generate_presentation` — and `ROI-bookends-master-v1..v4.pptx` — read by `tax_generate_bookends` (2026-09-25, DIRECT unit 3b): **v4 is live** (10,768,307 bytes, 9 slides, tokens `CLIENT_NAME` / `RISK_LABEL` / `RISK_NUM`, no speaker notes, built from the live ROI v10 by `scripts/roi-presentation/make_bookends_v4.py`), v1–v3 kept as rollbacks; **EIGHTEEN objects as of 2026-09-25** (derive the list — `supabase storage ls ss:///presentation-templates/ --experimental`; the three slashes, the trailing slash and `--experimental` are all required) — live read is **`ROI-template-master-v10.pptx`** (2026-09-25, v892, 29,691,770 bytes — 32 slides, 31 tokens / 52 sites: TPOM wording, `${{TAXES_WITH}}` on "Choose Who You Pay", two new token-free slides `slide31.xml` / `slide32.xml`; `REQUIRED_TEMPLATE_PARTS`), with **v9** as its rollback (2026-09-24, 29,670,862 bytes, md5 `c2e8a369…` after its same-name hot swap; still 31 tokens / 51 sites — three logos on every slide wired to the shared `ppt/media/logo_group.png` / `logo_member.png` parts, the new `slide30.xml` *"Additional Benefits"* slide, 30 slides; the handler refuses a master missing those parts, `REQUIRED_V9_PARTS`), with **v8** (the v821 rollback — *"VFO Tax Specialists"* header with `wrap="none"`, fee rows merged into one shape each, #480), v7 (the v820 wording-only version whose header wrapped, rollback for v820 only) / v6 (31 tokens, the pre-v820 rollback) / v5 (22 tokens) / v4 / v3 / v2 / v1 and **FOUR** dated hot-swap backups (two from 2026-08-17, plus `…-v5-pre-trim-2026-08-19.pptx` and `…-v5-pre-agenda-2026-08-19.pptx`) retained as rollbacks. A token-free deck change (styling, **and speaker notes — but `notesSlide23.xml` now carries `{{PLANNER_NOTES}}`, so an edit there is a hot swap only while that token's `<a:r>` run stays byte-intact**) ships by replacing the same object **after uploading a dated pre-edit backup**, a token/slide-count change needs a NEW versioned name + lockstep deploy, uploaded FIRST (#336/#342). **v4 has NO content delta** — it is byte-identical to `ROI-template-master-v3-pre-shift-2026-08-17.pptx`; the new name exists because restoring slide 24's `{{INVEST_YEARS}}` header is a token-SITE addition *relative to the live v3 object*. **v5 took the versioned path for the plainest possible reason: a token in a wholly new PART** (the first outside `ppt/slides/`); its two same-day follow-ups were hot swaps on that object. **v6 is the least ambiguous versioned case yet — it both ADDS token sites (nine) and RETIRES one (`{{FEE_HALF}}`), on five slides and two notes parts**, so a hot swap was never on the table; uploaded before the v795 deploy, v5 retained. **v9 took the versioned path** (new required parts, slide count and `sldIdLst`, moved slide-24 shapes the handler's thresholds measure), uploaded before its lockstep deploy with v8 retained; its content-only follow-up (slides 2/3/6) was a same-name hot swap. `supabase storage cp` cannot overwrite an existing key — use the Dashboard or the Storage API upsert (#409) |
...
| Upload (member's own headshot) | `member_self_profile_save` (`actions/members/self-profile-save.ts`) | `<ts>_member_<num>.jpg` (JPEG from the crop modal, ≤ 1.5 MB) |
| Upload / remove (member logo) | `member_branding_save` (`actions/members/branding-save.ts`) | `logo_member_<num>_<ts>.png` → `members.logo_image`; best-effort deletes the replaced `logo_member_` file |
| Upload / remove (group logo) | `tax_planning_group_logo_save` (`actions/tax-planners/group-logo-save.ts`) | `logo_group_<id>_<ts>.png` → `tax_planning_groups.logo_image`; best-effort deletes the replaced `logo_group_` file |
| Read (logos) | `tax_generate_presentation` (service role) | swaps the two logos into the ROI deck — [flows/tax-planning.md](../flows/tax-planning.md) |

**Logos (2026-09-24).** Server-named PNGs, all `cacheControl: "31536000"`, validated by `utils/logo-png.ts` (PNG magic, IHDR exactly **1200×400**, ≤ **1 MB**) on upload AND again when the deck reads them. The browser produces them (`src/components/shared/logoPng.js`, white rounded badge baked in; PNG/JPG/WebP accepted, SVG deliberately not). The delete-on-replace only ever touches its own prefix, so a headshot sharing the bucket is never removed.

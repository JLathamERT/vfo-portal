...
| `…/users/me/messages?q=filename:"<file>.pdf" to:<email>&maxResults=10` | GET | **2026-09-23** — `utils/gmail-thread.ts` `findThreadByAttachment(accessToken, fileName, toEmail)`: finds a thread by the PDF it CARRIED rather than its subject (several emails to one person can share a subject — MAP 1's member-pays agreement email and its invoice/receipt email both read *"Meeting with the Proactive Planning Team - …"*). Returns the OLDEST matching thread, or null; the caller then builds reply headers with `threadReplyHeaders`. Used only by the MAP 1 invoice/receipt **reissue** (`automation_CONTRACT_invoicereceipt` `reissue: true`, searching `REC-….pdf`), which falls back to the subject search above when it returns null. |
...
**Recipients are template-driven too (2026-07-03).** `email_templates.to_list` / `cc_list` / `bcc_list` hold a mix of raw emails and UPPERCASE role tokens (`CLIENT`, `MEMBER`, `ASSIGNED_PF`, `SPECIALIST`, `ADVISOR`, `ACCOUNTANT`, `TAX_PLANNER` (added 2026-07-22; **widened 2026-09-16 to resolve to TWO people — the plan's allocated tax planner AND its allocated team member**, via `taxPlanPeopleEmails(supabase, plan)`, chip label *"Tax Planner + allocated Team Member"* via `ROLE_LABELS`; a tax handler MUST pass `TAX_PLANNER` in its `resolveTemplateRecipients` ctx or the chip silently no-ops — gotcha #266; sandbox suppresses all Cc, so since v855 read the `sandbox: would have sent` log line instead), `TEAM_MEMBER`, `TEAM` = every `allowed_admins` row, `RECIPIENT` = the handler's built-in target), edited as chips in the Email Templates tab. Every templated handler resolves them per-send via `utils/email-recipients.ts resolveTemplateRecipients(supabase, tmpl, ctx, isSandbox, sandboxEmail, extraCc?, options?)` — the handler supplies `ctx` (which email each token means for THIS send); tokens with no ctx value are skipped; an empty resolved To falls back to `ctx.RECIPIENT`, so an email can never go unaddressed; sandbox still reroutes everything to the sandbox address with no Cc/Bcc. The old hardcoded counterparty Cc arrays (member/PF/tracy@vfo-services.com) were REMOVED from handlers — that routing now lives in the seeded template config (migration `email_templates_recipient_seeds`). Per-client Cc is appended through the same call's `extraCc` parameter — see the Additional Contacts paragraph below. The legacy `templateRecipients()` remains only for unwired hardcoded emails.

**A ctx value may be an ARRAY as of 2026-09-16.** `RecipientCtx` is `Partial<Record<string, string | string[] | null | undefined>>`; `resolveList` and the `RECIPIENT` fallback both flatten, so **any** role token can now address several people. `TAX_PLANNER` is the first to use it (planner + allocated team member). **When you widen a token this way, widen EVERY producer of it in the same change** — census them by grepping the single helper they all call, never by reading template rows — and check the bell side too: `notifyByRule`'s `dynamic` map already accepted arrays, so emails and bells drift apart silently (gotcha **#503**).
...
**And a PER-MEMBER leg as of 2026-09-16 — resolved CENTRALLY, inside `resolveTemplateRecipients` itself.** After the real To/Cc/Bcc are computed, `loadMemberContactCc` appends the flagged `member_contacts` rows of **any member whose own email already landed in To or Cc** (never Bcc — a hidden member stays hidden). **No handler was changed and no handler can forget it**, which is the whole structural difference from the client leg above. The compare on `members.email` is case-insensitive in JS (36 rows are mixed-case); the loader never throws. **Opting out is an explicit flag:** the new 7th `options` parameter's `{ skipMemberContacts: true }`, set on exactly the five credential handlers (`login-setup/send-email.ts`, `login-setup/request-reset.ts`, `advisor/login-setup-email.ts`, `accountant/login-setup-email.ts`, `onboarding/login-setup-email.ts`) — a login link is not something a Cc contact is entitled to. Derive the set: `grep -rn "skipMemberContacts"`.

**THE SANDBOX RECIPIENT LOG (v855) — how to verify any recipient change without sending.** Sandbox reroutes the whole send, so a recipient change is invisible by construction (#324) and every feature in this section shipped unverifiable. `resolveTemplateRecipients` now resolves the REAL recipients FIRST and logs

```
sandbox: would have sent to=[…] cc=[…] bcc=[…]
```

before returning the sandbox shape. **Non-sandbox output is byte-identical.** Read `function_logs` for `sandbox: would have sent`. Gotcha **#505**.
...
- **Adding a role-token chip to an existing template is only half the change.** The other half is the ctx in each sending handler. The canonical fix shape is `MEMBER: member?.email` + `ASSIGNED_PF: getPfEmail(client.assigned_pf || "")` — **in a TAX handler, `ASSIGNED_PF: await resolvePfEmail(supabase, client)` instead (2026-09-18, #514: on a Direct plan the PF is the member, and the select must carry `pf_member_number` too, #448)** — which usually also means widening the handler's `clients` `.select()` to include `member_number, assigned_pf` and adding a guarded members lookup.

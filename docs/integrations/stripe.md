...
**The refund** (the Tax Plan Red Light step → Refund, shown only on a Stop review since 2026-09-21) is the EXISTING `automation_TAX_depositrefund` path by PaymentIntent — `client_tax_plans.deposit_payment_intent_id`, which finalize copied from the intake row — full $500 on the primary account (see the stamp note above), `Idempotency-Key: deposit-refund-tax-<plan.id>` **rotated with `-r<epoch>` from `deposit_refund_failed_at` after a recorded failure** (2026-09-18, #489 — a fixed key would replay Stripe's error body for 24 h); only the EMAIL changed (addressed to whoever paid — see [tables/documents.md](../tables/documents.md)). A waived plan has nothing to refund and stops through `automation_TAX_stopnodeposit` instead — no Stripe call.

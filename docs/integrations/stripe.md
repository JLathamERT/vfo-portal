...
Two **new ERT webhook endpoints** (live + sandbox) were added on the **same function URL**, so four endpoints now deliver there (two accounts × two modes). Each is subscribed to **14 events**: `checkout.session.completed` / `.async_payment_succeeded` / `.async_payment_failed` / `.expired`, `payment_intent.succeeded` / `.processing` / `.payment_failed` / `.canceled`, `charge.dispute.created` / `.closed`, `charge.refunded`, `charge.refund.updated`, `refund.updated`, `refund.failed`. **The two VFO Services endpoints additionally carry `setup_intent.succeeded` / `.setup_failed` / `.canceled` (subscribed 2026-09-14, primary-only)** for the specialist licence continuation's micro-deposit path — a $0-today subscription collects its mandate through a SetupIntent, so none of the `payment_intent.*` events ever fire for it ([flows/specialist-license-continuation.md](../flows/specialist-license-continuation.md), #491).
...
7. **Specialist Onboarding background-check payments** (2026-06-03) — one-time Core ($350) or Max ($950) charge for the specialist's background check (Stage 3). ACH or Card; card grosses up the 2.9%+$0.30 fee. No `setup_future_usage`. Confirmation email at payment time; receipt + invoice PDFs on clearance. See [../flows/specialist-onboarding.md](../flows/specialist-onboarding.md). **Sibling (2026-09-23): admin-sent Background Check payment REQUESTS** — any amount, one-time, primary account, a FRESH customer per request, `metadata.pipeline=SPECIALIST_BG_CHECK` + `payment_kind=bg_check_request` + `request_id` + `checkout_token` on BOTH session and PI, ACH pinned to instant verification; routed by metadata ahead of the customer cascade, table `specialist_bg_requests`. See [../flows/specialist-bg-requests.md](../flows/specialist-bg-requests.md).
...
| `metadata.payment_kind` | (none — uses `payment_number` for quarterly) | `retainer` / `implementation` / **`tax_intake_deposit`** *(2026-09-17 — the $500 intake deposit, card or ACH; routed by the session id / `metadata.intake_id` → `tax_intake_requests`, NO customer; also carries `deposit_method`)* | `onboarding` / **`onboarding_deposit`** / **`onboarding_balance`** *(2026-09-04)* | `onboarding` / **`onboarding_deposit`** / **`onboarding_balance`** *(2026-09-04)* | `purchase` | (none — uses `member_number` + `credits`) | `background_check` (+ `bg_type=core\|max`) **or `license`** (`mode=subscription`, $99/mo) | **`card_update`** (`mode=setup`, no charge) |
...
**One builder since 2026-09-23:** `tax_intake_deposit_checkout` ([actions/tax/intake-deposit-checkout.ts](C:/vfo-edge-functions/supabase/functions/vfo-admin-api/actions/tax/intake-deposit-checkout.ts)), called from the public `/tax-deposit-pay?token=` card-or-ACH choice page. `tax_intake_submit` (route A), `tax_intake_link_submit` (route B) and a confirmed Tax Diagnostic's **Pay deposit** email all send the payer there; none of them calls Stripe. The body is `{token, method}` — the `tax_intake_requests` row found by the token is the whole credential (#310):

```
mode: payment
# NO customer — the client does not exist yet (no clients row, no Stripe customer)
customer_email: <payer's address of record>       # members.email when payer='member', else the invited client email
payment_method_types: ["card"]  or  ["us_bank_account"]   # the payer's choice
line_items[0]:
  price_data.currency: usd
  price_data.unit_amount: 51524 (card)  or  50000 (ACH)
      # card = cardChargeCents(500) from constants/card-fee.ts:
      #   round((500 + 0.30) / (1 - 0.029) * 100) -> $515.24, the /tax-pay formula
  price_data.product_data.name: "VFO Tax Planning Deposit - Client: <First Last> - Member: (<member_number>) <Member Name>"
# ACH only: NO payment_method_options.us_bank_account.verification_method pin (#298) —
#   bank sign-in first, manual entry (micro-deposits) as the fallback
custom_text.submit.message: ACH_BANK_SIGN_IN_NOTE (ACH only)
success_url: route A  <origin>/member?tab=msm_tax&intake=<id>&paid=1[&ach=1]
             else     <origin>/tax-deposit-pay?token=…&paid=1[&ach=1]
cancel_url:  <origin>/tax-deposit-pay?token=…&canceled=1
payment_intent_data.description: <same memo>
metadata AND payment_intent_data.metadata (the same five keys, #473):
  pipeline:       TAX
  payment_kind:   tax_intake_deposit
  intake_id:      <tax_intake_requests.id>
  member_number:  <member_number>
  deposit_method: card | ach
```

**The fee helper.** [constants/card-fee.ts](C:/vfo-edge-functions/supabase/functions/vfo-admin-api/constants/card-fee.ts) (`CARD_FEE_PERCENT` 0.029, `CARD_FEE_FIXED` 0.30, `cardChargeCents(base)`) is the one shared definition of the house card gross-up, identical to the `/tax-pay` card leg; `actions/specialist-bg/shared.ts` re-exports it under the same name. The deposit's `tax_intake_deposit_load` computes `card_total` / `card_fee` from it so the page does no arithmetic. The other inline copies of the formula (the `/pay`, `/advisor-pay`, `/pip-pay` … builders) are deliberately not refactored onto it. `deposit_amount_cents` stays the 50000 base; the fee the payer covered is recorded on `tax_intake_requests.deposit_card_fee`.

**Where the account is decided:** `loadSandboxConfigForMember(sb, "TAX", memberNumber)` — the **member-keyed** sandbox helper, because no client exists to key on (it is also what forces Test Member 59524 into sandbox for the whole route). It is re-derived **at the moment the checkout mints** and stamped on `tax_intake_requests.stripe_account` + `sandbox` (#485: config mints, the stamp governs). **The stamp is read since 2026-09-23:** `rowStripeKey` uses it to expire a stale session and to read the PaymentIntent in the webhook, and the webhook refuses a row not stamped on the primary account or whose `sandbox` disagrees with `event.livemode`. The later $500 refund still runs through the EXISTING `automation_TAX_depositrefund`, which resolves the client-keyed `TAX` sandbox config and reads `STRIPE_SECRET_KEY[_SANDBOX]` inline — the PRIMARY account, correct only because `TAX` is `'vfos'`. The session id is written back to `stripe_checkout_session_id` (unique). **No deposit is minted at all when the member's waiver applies** (two qualifying tax clients) — the row is `waived` and finalized directly, at submit or, if the member crossed the line since, at the checkout.

**Re-minting.** A row is payable while `pending` / `expired` (or a diagnostic `invited` row) with no ACH in flight (`deposit_processing_at`). Before minting, any previously stamped session is expired with the stamp's key: Stripe reporting it `complete` → **409** (money in flight), its state unknown → **500** (never two payable sessions for one deposit). A Stripe error leaves the row untouched and payable; a failed stamp write expires the fresh session.

**Webhook:** one primary-account block keyed on `metadata.payment_kind='tax_intake_deposit'` (session AND PI), ahead of the customer-keyed cascade — `utils/tax-intake-deposit-webhook.ts`. The PaymentIntent's own status decides (#475): a **card** (`succeeded` at `checkout.session.completed`) is booked `paid` with method / last4 / `deposit_card_fee` and finalized at once; an **ACH** (`processing` / `requires_action`) is booked in flight — row stays `pending`, `deposit_processing_at` (+ `deposit_bank_verification_pending_at` for manual entry), the ACH confirmation email at submit — and finalized only at **`payment_intent.succeeded`**. `payment_intent.payment_failed` / `.canceled` / `checkout.session.async_payment_failed` on a BOOKED PI release the row to `expired` (`deposit_failed_at`, the payer's link works again) and raise `FAILURE_tax_intake_deposit_ach`; an unbooked PI (a page opened and abandoned, #482) is ignored. `checkout.session.expired` marks a `pending` row with no booked PI `expired`. The two generic failure resolvers skip this `payment_kind`. Detail: [../flows/stripe-webhook.md](../flows/stripe-webhook.md) A1½.

**The refund** (the Tax Plan Red Light step → Refund, shown only on a Stop review since 2026-09-21) is the EXISTING `automation_TAX_depositrefund` path by PaymentIntent — `client_tax_plans.deposit_payment_intent_id`, which finalize copied from the intake row — on the primary account (see the stamp note above), **for the $500 base only** when the plan has an intake row (`amount` = `min(deposit_amount_cents, PI amount)`, 2026-09-23, so a card payer's $15.24 fee is kept; a legacy hand-pasted plan refunds the full PI), `Idempotency-Key: deposit-refund-tax-<plan.id>` **rotated with `-r<epoch>` from `deposit_refund_failed_at` after a recorded failure** (2026-09-18, #489 — a fixed key would replay Stripe's error body for 24 h); the email is addressed to whoever paid — see [tables/documents.md](../tables/documents.md). A waived plan has nothing to refund and stops through `automation_TAX_stopnodeposit` instead — no Stripe call.
...
Independent of the per-pipeline `pipeline_sandbox_config` toggle, every client-scoped TAX/MAP1/PIP money/email/BoldSign handler now resolves its mode via `loadSandboxConfigForClient(sb, pipeline, clientId)` (`integrations/sandbox-config.ts`) instead of the bare `loadSandboxConfig`. It reads the global row, and if the pipeline is LIVE it force-flips ONLY cases whose client belongs to a `constants/test-sandbox.ts TEST_SANDBOX_MEMBER_NUMBERS` member (currently `59524`) to sandbox (sandbox Stripe key + BoldSign test + `sandbox_email` redirect). Force-ON-only and fail-safe: any lookup failure returns the unchanged global result, so it can never push a real case to live. It exists so Jake can exercise the live automations end-to-end on the test member without a real charge. **The member-keyed twin, 2026-09-16:** membership is keyed on `member_number`, not `client_id`, so it was never covered — `isTestSandboxMember(memberNumber)` (constant check, no DB read) and `loadSandboxConfigForMember(sb, pipeline, memberNumber)` now do the same force-ON-only job for `MEMBER_MEMBERSHIP`, swapped in at every member-keyed read including `send-setup-link.ts`, the site that STAMPS `plan.sandbox` at mint. `sweep.ts` and `utils/membership-setup-reminder.ts` keep their single global read but OR the check into every per-plan USE. Webhooks, `stripe-remap`, `plans-load` and `arrears-digest` stay out of it — they key off the row stamp. **Never force a member-keyed pipeline by stamping the row alone: every mode-mismatch guard then trips and the charge is silently skipped (#504).** **The specialist twin, 2026-09-23:** `TEST_SANDBOX_EXPERT_IDS = [6137]` (the production "Test Specialist", `experts.id`) in the same file — `specialist_bg_request_send` ORs it into the `SPECIALIST_ONBOARDING` toggle read at mint, and every later step of a Background Check payment request follows the row's own `sandbox` stamp (no mode-mismatch guard reads the live toggle there, so stamping at mint is safe). Scoped to that one pipeline only. 
...
| Card processing fee constants | `2.9%` and `$0.30` — embedded in the gross-up math at line 1116 and PayPage:79; since 2026-09-23 also shared as `CARD_FEE_PERCENT` / `CARD_FEE_FIXED` / `cardChargeCents()` in `constants/card-fee.ts` (used by the tax intake deposit and the Background Check requests; the older builders keep their inline copies) |

...
The top-level picture. Two repos, one Supabase project, four external integrations, one static-hosted SPA. The whole system is held together by a **123-line orchestrator** in `vfo-admin-api/index.ts` that dispatches to **496 actions across 508 modular action handler files** *(v: 2026-09-11 — derive both, do not trust them)* plus two routers (`router/dispatch.ts`, `router/webhooks.ts`).
...
└──────────────────────────────┬─────────────────────────────────┬─────────────────┘
                          ...
                          │   (496 actions, 123-line orchestrator        │

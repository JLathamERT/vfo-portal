...
The top-level picture. Two repos, one Supabase project, four external integrations, one static-hosted SPA. The whole system is held together by a **123-line orchestrator** in `vfo-admin-api/index.ts` that dispatches to **533 actions across 536 modular action handler files** *(v: 2026-09-23 — derive both, do not trust them)* plus two routers (`router/dispatch.ts`, `router/webhooks.ts`).
...
└──────────────────────────────┬─────────────────────────────────┬─────────────────┘
                          ...
                          │   (533 actions, 123-line orchestrator        │
                          │    + 536 handler files + 2 routers)          │
                          │                                              │

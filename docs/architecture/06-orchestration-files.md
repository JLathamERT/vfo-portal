...
| `vfo-admin-api/router/dispatch.ts` | ~700 lines | The two action-dispatch tables: `PUBLIC_HANDLERS` (**145 entries**) + `AUTH_HANDLERS` (**363 entries**) = 508 dispatched, +6 logins in `index.ts` = the **514** live count. _(Counts re-verified 2026-09-18 by counting `(c) =>` inside each table block — re-count, never trust this line. That day's additions: PUBLIC `tax_intake_link_load` / `tax_intake_link_submit`; AUTH `tax_intake_eligibility` / `_submit` / `_holistic_submit` / `_load` / `_send_link`; and `tax_load_plans` / `tax_load_progress` / `tax_load_specialists` now pass `c.auth`.)_ Each entry adapts a handler from `actions/<group>/<file>.ts` to a uniform ctx-based signature. (2026-06-16: 6 Tax actions moved PUBLIC→AUTH; +3 Phase D card-change regs — `payments_send_card_update` AUTH/superadmin, `payments_loadcardupdate` + `payments_cardupdate_checkout` PUBLIC.) |
...
| `vfo-admin-api/actions/<group>/*.ts` | ~300 files | One file per action handler. Groups: `auth/`, `data/`, `client-notes/`, `member-program-notes/`, `email-templates/`, `notifications/`, `admins/`, `member-logins/`, `vault/`, `members/`, `specialists/`, `gc/`, `coaching/`, `tax/` (60 files as of 2026-09-18, incl. the seven `intake-*.ts` handlers), `ciq/`, `onboarding/`, `msm/` (~45 files incl. PIP), `pipeline/` (17 files), `regular/`, `pft/`, `advisor/` (22 files incl. `save-team-member.ts`), `accountant/` (23 files incl. `save-partnership.ts` + `save-team-member.ts`), `payments/` (Payments-tab read aggregators `{member,client,specialist}-payments-load.ts` + `all-payments-load.ts` + shared `normalize.ts`, plus the **Phase D** admin card/bank-change trio `card-update-send.ts` / `card-update-load.ts` / `card-update-checkout.ts` over shared `card-update-shared.ts`, added 2026-06-16). |
| `vfo-admin-api/utils/`, `constants/`, `types/`, `integrations/` | **74 files in `utils/`** + constants + types + integrations (v: 2026-09-18 — `ls` it, never trust this number) | Shared helpers: `cors.ts`, `crypto.ts`, `format-date.ts`, `html-templates.ts`, `json.ts`, `pf-emails.ts`, `member-number.ts` (category-relative `nextMemberNumber`), `new-model-sale-email.ts` (`sendNewModelSaleEmail` — internal "New Model Sale" team draft on advisor/accountant create), `client-ownership.ts` (C2 `denyIfNotOwnClient` / `denyIfNotOwnEnrollment` member→resource guards, added 2026-06-18; **`denyIfNotOwnPlan`** plan→client→member, 2026-09-18), **the 2026-09-17 tax-intake set** — `tax-intake-questions.ts` (the 37 questions + validation, mirrored by the frontend `taxIntakeQuestions.js`), `tax-direct-eligibility.ts` (`qualifyingTaxClients` — the one predicate behind the $500 deposit waiver and unit-2 Direct eligibility), `tax-intake-finalize.ts` (`finalizeTaxIntake` — enrollment/client/junction/plan/Deposit Paid step/confirmation, three latches), `tax-deposit-docs.ts` (the deposit INV/REC PDF pair), `tax-deposit-team-share.ts` (the $250 Connect transfer to the planning group, own status vocabulary), `tax-intake-request-email.ts` (the Holistic "complete the form" ask, template 276), `tax-intake-link-email.ts` (the client-link email, template 277), `email-signature.ts` (`VFO_SIGNATURE` + the delivery-layer footer guard `ensureSignature` / `ensureSignatureEncoded`; `gmail-draft.ts` re-exports the constant for its ~118 importers), and `constants/member-types.ts` (`ACCOUNTANT_TYPES` / `isAccountantMember`, the first server-side copy of the frontend `memberTypes.js` vocabulary), `login-throttle.ts` (H1 `checkLoginThrottle` / `recordLoginFailure` / `clearLoginFailures` over the `login_attempts` table, added 2026-06-18), `tax-plan-steps.ts` (the shared tax step machine — `buildTaxPlanSteps` / `taxPlanStepsComplete`, extracted verbatim from `actions/clients/overview-tax.ts` on 2026-08-17 so the planner portal reuses it rather than mirroring it), role-gate arrays, `constants/onboarding-team.ts` (`teamMemberRecipient` — onboarding team-member name → @elitert login for notification routing), `JsonResponder` / `AuthContext` / ctx types, integration scaffolding for Stripe/BoldSign/Google. |
...
| [src/pages/TaxIntakePage.jsx](src/pages/TaxIntakePage.jsx) | 109 lines | `/tax-intake?token=...` | **2026-09-17.** Token-link page for the CLIENT route of the tax intake (`tax_intake_link_load` → `tax_intake_link_submit`). Raw `fetch` (no session), on `TokenShell`; mounts [TaxIntakeForm](src/components/member/TaxIntakeForm.jsx) in `publicMode`. `?paid=1` (the Checkout return) is checked FIRST and shows the thank-you. |
...
| [src/components/member/TaxIntakeForm.jsx](src/components/member/TaxIntakeForm.jsx) + [taxIntakeQuestions.js](src/components/member/taxIntakeQuestions.js) | 398 + 128 lines | MemberMSMTracking (Tax Planning tab, "+ Add new tax client" and `?intake_client=`) and TaxIntakePage (`publicMode`) | **2026-09-17.** The 37-question Tax Planning Form: chooser (fill it in / email the client a link), form (`tax_intake_eligibility`, `tax_intake_submit` or `tax_intake_holistic_submit`), link mini-form (`tax_intake_send_link`), Test Member 59524 fill button. `taxIntakeQuestions.js` is the FRONTEND HALF of a cross-repo contract with the backend `utils/tax-intake-questions.ts` — same ids, labels, options and the same two validation-message templates; the two files move together. |

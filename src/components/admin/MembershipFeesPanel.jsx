...
export default function MembershipFeesPanel({ title, category, allMembers = [], isSuperadmin = false, initialMemberNumber = null }) {
  ...
  return (
    <div style={{ padding: '24px', maxWidth: '1050px', margin: '0 auto', fontFamily: 'Inter, sans-serif' }}>
      ...
      {/* Jake-only maintenance tool. NOT gated on the isSuperadmin prop above — at the
          Accounting mount that prop is canSeeTab('accounting'), and membership_stripe_remap
          is SUPERADMIN_ONLY on the server (403 for anyone else). */}
      {!!getSession()?.is_superadmin && <StripeRemapSection />}
    </div>
  )
}

// ── Move plans to ERT Stripe (superadmin) ────────────────────────────────────
//
// After Jake runs Stripe's "copy customer data to another account" (VFO Services
// → ERT), the ERT Dashboard's Documents area offers a mapping CSV whose header is
// exactly `customer_id_old,source_id_old,customer_id_new,source_id_new`. Pasting
// it here re-points each membership plan at its new ERT customer + payment method.
// Dry run first, always: the Apply button stays dead until a dry run of THESE
// exact textarea contents came back with at least one would_move.

const REMAP_HEADER = ['customer_id_old', 'source_id_old', 'customer_id_new', 'source_id_new']
const REMAP_BATCH = 200

function parseRemapCsv(text) {
  const lines = String(text || '').trim().split(/\r?\n/).map(l => l.trim()).filter(l => l !== '')
  if (lines.length === 0) return { rows: [], error: '' }
  const head = lines[0].split(',').map(c => c.trim().toLowerCase())
  if (head.length !== REMAP_HEADER.length || !REMAP_HEADER.every((h, i) => head[i] === h)) {
    return { rows: [], error: 'The first line must be the mapping header: ' + REMAP_HEADER.join(',') }
  }
  const rows = []
  for (let i = 1; i < lines.length; i++) {
    const parts = lines[i].split(',').map(c => c.trim())
    if (parts.length !== REMAP_HEADER.length) {
      return { rows: [], error: `Line ${i + 1} has ${parts.length} field${parts.length === 1 ? '' : 's'}, expected ${REMAP_HEADER.length}.` }
    }
    rows.push({ customer_id_old: parts[0], source_id_old: parts[1], customer_id_new: parts[2], source_id_new: parts[3] })
  }
  return { rows, error: '' }
}

const REMAP_OUTCOME_COLOR = {
  moved: '#16a34a',
  would_move: '#16a34a',
  already_ert: '#6b7280',
  error: '#ef4444',
...
function remapOutcomeColor(outcome) {
  return REMAP_OUTCOME_COLOR[outcome] || '#e06717'
}

function StripeRemapSection() {
  const [open, setOpen] = useState(false)
  ...
  // The exact textarea contents a dry run approved. Any edit makes this stale,
  // so Apply can never run against text the server has not previewed.
  const [dryOkFor, setDryOkFor] = useState(null)

  const parsed = useMemo(() => parseRemapCsv(text), [text])
  const canDryRun = !busy && parsed.rows.length > 0 && !parsed.error
  const canApply = canDryRun && dryOkFor === text
  ...
  async function run(dryRun) {
    setBusy(true); setErr(''); setResults(null); setSummary(null)
    const merged = { moved: 0, would_move: 0, skipped: 0 }
    const all = []
    try {
      for (let i = 0; i < parsed.rows.length; i += REMAP_BATCH) {
        const res = await callApi('membership_stripe_remap', {
          mappings: parsed.rows.slice(i, i + REMAP_BATCH),
          dry_run: dryRun,
        })
        if (res?.error) throw new Error(res.error)
        const s = res?.summary || {}
        merged.moved += s.moved || 0
        merged.would_move += s.would_move || 0
        merged.skipped += s.skipped || 0
        all.push(...(res?.results || []))
      }
      setSummary(merged)
      setResults(all)
      // A dry run only unlocks Apply when there is actually something to move.
      // After a real Apply, require a fresh dry run before the next one.
      setDryOkFor(dryRun && merged.would_move > 0 ? text : null)
    } catch (e) {
      setErr(e?.message || 'The remap call failed.')
      setResults(all.length ? all : null)
      setSummary(all.length ? merged : null)
      setDryOkFor(null)
    } finally {
      setBusy(false)
    }
  ...
  return (
    <div style={{ ...card, marginTop: '28px', overflow: 'hidden' }}>
      <div onClick={() => setOpen(o => !o)}
        ...
        <div style={{ flex: 1, minWidth: 0 }}>
          <div style={{ fontSize: '14px', fontWeight: 700, color: 'var(--vfo-ink)' }}>Move plans to ERT Stripe</div>
          <div style={{ fontSize: '12px', color: 'var(--vfo-muted)', marginTop: '2px' }}>Superadmin only · paste Stripe's copied-customer mapping CSV</div>
        </div>
      </div>

      {open && (
        <div style={{ padding: '0 18px 18px', borderTop: '1px solid var(--vfo-border-soft)' }}>
          <p style={{ fontSize: '13px', color: 'var(--vfo-muted)', lineHeight: 1.6, margin: '14px 0 4px' }}>
            Run Stripe's copy-customer-data job from VFO Services to ERT first, then download the mapping CSV from the ERT Dashboard's Documents area and paste it below.
          </p>
          <p style={{ fontSize: '13px', color: 'var(--vfo-muted)', lineHeight: 1.6, margin: '0 0 14px' }}>
            Always Dry run first — Apply stays disabled until a dry run of exactly this text comes back with something to move.
          </p>

          <div style={label}>Mapping CSV</div>
          <textarea
            value={text}
            onChange={e => onText(e.target.value)}
            rows={8}
            spellCheck={false}
            placeholder={REMAP_HEADER.join(',') + '\ncus_old123,pm_old123,cus_new123,pm_new123'}
            style={{ ...input, fontFamily: 'ui-monospace, SFMono-Regular, Menlo, monospace', fontSize: '12px', lineHeight: 1.5, resize: 'vertical' }}
          />

          <div style={{ display: 'flex', alignItems: 'center', gap: '12px', flexWrap: 'wrap', marginTop: '12px' }}>
            <button type="button" disabled={!canDryRun} onClick={() => run(true)}
              style={{ ...ghostBtn, opacity: canDryRun ? 1 : 0.5, cursor: canDryRun ? 'pointer' : 'not-allowed' }}>
              {busy ? 'Working…' : 'Dry run'}
            </button>
            <button type="button" disabled={!canApply} onClick={() => run(false)}
              style={{ ...primaryBtn(!canApply), cursor: canApply ? 'pointer' : 'not-allowed' }}>
              Apply
            </button>
            <span style={{ fontSize: '12px', color: 'var(--vfo-faint)' }}>
              {parsed.error
                ? ''
                : `${parsed.rows.length} mapping${parsed.rows.length === 1 ? '' : 's'} parsed${parsed.rows.length > REMAP_BATCH ? ` · sent in ${Math.ceil(parsed.rows.length / REMAP_BATCH)} batches of ${REMAP_BATCH}` : ''}`}
            </span>
          </div>

          {parsed.error && (
            <div style={{ marginTop: '12px', fontSize: '12.5px', padding: '8px 12px', borderRadius: '8px', color: '#b91c1c', border: '1px solid #fecaca', background: '#fef2f2' }}>{parsed.error}</div>
          )}
          {err && (
            <div style={{ marginTop: '12px', fontSize: '12.5px', padding: '8px 12px', borderRadius: '8px', wordBreak: 'break-all', color: '#b91c1c', border: '1px solid #fecaca', background: '#fef2f2' }}>{err}</div>
          )}

          {summary && (
            <div style={{ marginTop: '14px', fontSize: '13px', fontWeight: 600, color: 'var(--vfo-ink-2)' }}>
              {summary.moved} moved · {summary.would_move} would move · {summary.skipped} skipped
            </div>
          )}
          ...
          {results && results.length > 0 && (
            <div style={{ marginTop: '10px', overflowX: 'auto' }}>
              <table style={{ width: '100%', borderCollapse: 'collapse', fontSize: '12.5px' }}>
                <thead>
                  <tr>
                    {['Plan', 'Member', 'Customer', 'Outcome', 'Detail'].map(h => (
                      <th key={h} style={{ textAlign: 'left', padding: '8px 10px', borderBottom: '1px solid var(--vfo-border-soft)', color: 'var(--vfo-muted)', fontSize: '10.5px', fontWeight: 700, textTransform: 'uppercase', letterSpacing: '1px', whiteSpace: 'nowrap' }}>{h}</th>
                    ))}
                  </tr>
                </thead>
                <tbody>
                  {results.map((r, i) => (
                    <tr key={`${r.plan_id || 'x'}-${i}`}>
                      <td style={remapCell}>{r.plan_id ?? '—'}</td>
                      <td style={remapCell}>{r.member_number || '—'}</td>
                      <td style={{ ...remapCell, wordBreak: 'break-all' }}>{r.customer_id || '—'}</td>
                      <td style={remapCell}><StatusPill label={r.outcome || '—'} color={remapOutcomeColor(r.outcome)} /></td>
                      <td style={{ ...remapCell, color: 'var(--vfo-muted)' }}>{r.detail || '—'}</td>
                    </tr>
                  ))}
                </tbody>
              </table>
...
const remapCell = { padding: '8px 10px', borderBottom: '1px solid var(--vfo-border-soft)', color: 'var(--vfo-ink)', verticalAlign: 'top' }
